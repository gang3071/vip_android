package com.timego.calculcator.driver;
...
import com.timego.calculcator.BuildConfig;
import com.timego.calculcator.LogUtils;
import com.timego.calculcator.utils.SerialInputOutputManager;
...
// usb 数据传输工具
public class USBTransferUtil1 {

    private String TAG = "USBTransferUtil1";
...
// 单例 -------------------------
    private static USBTransferUtil1 usbTransferUtil;
    public static USBTransferUtil1 getInstance() {
        if(usbTransferUtil == null){
            usbTransferUtil = new USBTransferUtil1();
        }
...
//                    boolean granted = intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false);  // 不知为何获取到的永远都是 false 因此无法判断授权还是拒绝
    ...
    public void refreshDevice(){
        ...
        LogUtils.i("当前可用 usb 设备数量XXXUSBTransferUtil1: " + availableDrivers.size() );
        // 有设备可以连接
...
//            // 通常手机只有充电口 1 个
//            else {
                usbSerialDriver = availableDrivers.get(1);
//            }
            usbSerialPort = usbSerialDriver.getPorts().get(0);  // 一般设备的端口都只有一个，具体要参考设备的说明文档
            LogUtils.i("XXXUSBTransferUtil1:"+usbSerialDriver.getPorts().size());

            LogUtils.i("XXXUSBTransferUtil1:"+usbSerialPort.getDevice().getDeviceName());
            // 同时申请设备权限
    ...
    public void startReceiveData(){
        ...
        inputOutputManager = new SerialInputOutputManager(usbSerialPort, new SerialInputOutputManager.Listener() {
            ...
            // 在这里处理接收到的 usb 数据 -------------------------------
                ...
                String data_str1 = bytes2string(readBuffer);
                LogUtils.i( "收到 usb 数据00: " + data_str1);
//                for (int i = 0;i<data_str1.length();i++){
//                     name+=(asciiToNumber(data_str1.charAt(i))+"");
//                }
//                Log.i(TAG, "收到 usb 数据00: " + name);
                LogUtils.i("收到 usb 数据11: " + HexDump.toHexString(data));
//                RxBus.getInstance().post(new RxBusBean(RxBusBean.TYPE_UPDATE_LIST_LOG,"出钞机收到数据："+ HexDump.toHexString(data)));

                if (data.length > 0) {
                    if(HexDump.toHexString(data).contains("06")){
                        if (onUSBDateReceive != null) {
                            LogUtils.i( "收到 usb 数据11:去回调 " );

                            onUSBDateReceive.onNextMoney();
                        }
                    }else if(HexDump.toHexString(data).contains("81 40")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("100");
                        }
                    }else if(HexDump.toHexString(data).contains("81 41")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("200");
                        }
                    }else if(HexDump.toHexString(data).contains("81 42")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("500");
                        }
                    }else if(HexDump.toHexString(data).contains("81 43")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("1000");
                        }
                    }else {
                        if (HexDump.toHexString(data).contains("90 06 12") || HexDump.toHexString(data).contains("90 05 50")) {
                            name = HexDump.toHexString(data).replace(" ", "");
                        } else {
                            name = name + HexDump.toHexString(data).replace(" ", "");
                            LogUtils.i("收到 usb 数据1122: " + name.replace(" ", ""));
                            if (name.contains("9006120303AE")) {
                                if (onUSBDateReceive != null) {
                                    onUSBDateReceive.onReceive("收钱10");
                                }
                            } else if (name.contains("9006120403AF")) {
                                if (onUSBDateReceive != null) {
                                    onUSBDateReceive.onReceive("收钱50");
                                }
                            }
                            name = "";
                        }
    ...
    public void write(String data_hex){
        if(usbSerialPort != null){
            ...
            if(usbSerialPort.isOpen()){
                byte[] data_bytes = hex2bytes(data_hex);  // 將字符数据转化为 byte[]
