...
//                    boolean granted = intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false);  // 不知为何获取到的永远都是 false 因此无法判断授权还是拒绝
    ...
    public void refreshDevice(){
        ...
        LogUtils.i("当前可用 usb 设备数量XXX: " + availableDrivers.size() );
        // 有设备可以连接
...
//            }
            ...
            LogUtils.i("XXX:"+usbSerialPort.getDevice().getDeviceName());
            // 同时申请设备权限
...
//                Log.i(TAG, "收到 usb 数据00: " + name);
                String datts = HexDump.toHexString(data);
//                if(!datts.equals("80 8F")||!datts.equals("00")) {
//                    LogUtils.i("收到 usb 数据11: " +datts);
//                }
//                RxBus.getInstance().post(new RxBusBean(RxBusBean.TYPE_UPDATE_LIST_LOG,"纸钞机收到数据："+ HexDump.toHexString(data)));

                if (data.length > 0) {
                    if(datts.contains("06")){
                        if (onUSBDateReceive != null) {
                            LogUtils.i( "收到 usb 数据11:去回调 " );
                            onUSBDateReceive.onNextMoney();
                        }
                    }else if(datts.contains("81 40")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("100");
                        }
                    }else if(datts.contains("81 41")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("200");
                        }
                    }else if(datts.contains("81 42")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("500");
                        }
                    }else if(datts.contains("81 43")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("1000");
                        }
                    }else if(datts.startsWith("10")){
                        if (onUSBDateReceive != null) {
                            onUSBDateReceive.onReceive("-7");
                        }
                    }else {
